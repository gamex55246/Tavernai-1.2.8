...
var api_scale = "https://dashboard.scale.com/spellbook/api/v2/deploy/owarida";
...
app.post("/saveSummary", jsonParser, function(request, response){
	var summary = request.body.summary;
    var dir_name = String(request.body.avatar_url).replace('.png','');
    fs.writeFile(chatsPath+dir_name+"/"+request.body.file_name+'summary.json', JSON.stringify({ summary: summary }), function(err) {
        if(err) {
            response.send(err);
            return console.log(err);
            //response.send(err);
...
app.post("/loadSummary", jsonParser, function(request, response){
    var dir_name = String(request.body.avatar_url).replace('.png','');
    // Read the summary data from file
    fs.readFile(chatsPath+dir_name+"/"+request.body.file_name+'summary.json', function(err, data) {
        if (err) {
            console.error(err);
...
    api_key_openai = request.body.key;
    ...
    client.get(api_openai+"/models",args, function (data, response) {
        ...
        if(response.statusCode == 401){
            cconsole.log('Access Token is incorrect (openAI).', data);
